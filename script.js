...
function getMovies(keyword = '') {
    ...
    movies.forEach((listMovies) => {
        let movieTitle = String(listMovies.title).toUpperCase().trim()
        let searchValue = String(keyword).toUpperCase().trim()

        if (movieTitle.includes(searchValue)) {
            results.push(listMovies)
        } 
    })
    console.log(results)
    return results;
...
function showMovies(keyword = '') {
  ...
  let noMovies = document.getElementById('error-not-found')
  if (!movies.length) {
    //ketika movie tdk ada
    ...
    <div class="col">
            <div class="p-3">
                <div class="card" style="width: auto">
                    <img src="${movies[i].poster_path}" class="card-img-top" alt="${movies[i].title}">
                    <div class="card-body">
                    <div class="row">
                      <h5 class="col-9 card-title">${movies[i].title}</h5>
